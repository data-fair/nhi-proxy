...
**Spec:** `docs/superpowers/specs/2026-09-10-nhi-proxy-design.md` — read it alongside this plan; every task below argues from it.
...
- **Secrets are 0600 in a 0700 directory** under `~/.config/nhi-proxy/<profile>/`, never inside a project tree.
- **No credential is ever logged, printed, or placed in a response body** — except the *public* JWKS that `setup` prints for the admin.
- **Profiles are a first-class concept.** One directory per profile under `~/.config/nhi-proxy/`; a profile is any subdirectory containing a `config.json`. There is no registry file. Profile names default to the site host (`koumoul.com`), never a generic `default`.
...
- License `AGPL-3.0-only`, author `Alban Mouton <alban.mouton@gmail.com>`, package name `@data-fair/nhi-proxy`, matching the rest of the stack.
...
| File | Responsibility |
|---|---|
| `bin/nhi-proxy.ts` | Shebang entry; `parseArgs` dispatch to commands. No logic. |
...
- Consumes: nothing.
- Produces:
  - `profileDir(profile: string): string` — `~/.config/nhi-proxy/<profile>` (the name is **required**; resolution happens in `profiles.ts`)
  ...
  - `class NoProfilesError extends Error` — thrown by `resolveProfile` when none exist, so bare `nhi-proxy` can catch it and run `setup`
...
const cfg = (site: string, port: number) => JSON.stringify({
  site, sdPath: '/simple-directory', issuer: 'https://nhi-proxy.data-fair.cloud/x',
  subject: 'a@b', port
})
...
const holderFor = async (origin: string, handlerExtras: Partial<any> = {}) => {
  ...
  return new SessionHolder({
    config: {
      ...
      issuer: 'https://nhi-proxy.data-fair.cloud/test', subject: 'test@host', ...handlerExtras
    },
    key,
    kid
  })
...
test('an exchange writes nothing to the profile directory', async () => {
  ...
  const holder = new SessionHolder({
    config: {
      ...
      issuer: 'https://nhi-proxy.data-fair.cloud/test', subject: 'test@host'
    },
    key,
    kid
  })
...
- A failed refresh returns **502 with the nhi-proxy cause as the body**; the request is never forwarded unauthenticated.
...
**Files:**
- Create: `bin/nhi-proxy.ts`, `src/commands/setup.ts`, `src/commands/enroll.ts`, `src/commands/serve.ts`, `src/commands/status.ts`, `src/commands/ca.ts`, `src/commands/profiles.ts`
- Test: `src/commands/setup.test.ts`

**Interfaces:**
- Consumes: everything from Tasks 1–7.
- Produces: the `nhi-proxy` binary with `setup`, `enroll`, `serve`, `status`, `ca`, `profiles`, plus bare-command dispatch.

Three behaviours from spec §7.1 and §11 drive this task:

- **Bare `nhi-proxy`** runs `setup` with 0 profiles, `serve` with 1, and lists with 2+. A single unenrolled profile is *not* a special case — `serve` already fails with "not enrolled yet, run `nhi-proxy enroll <client_id>`", which points at the next step without a branch.
...
export const runSetup = async (opts: SetupOptions) => {
  ...
  await writeConfig(profile, {
    ...
    issuer: previous?.issuer ?? `https://nhi-proxy.data-fair.cloud/${randomBytes(4).toString('hex')}`,
...
node bin/nhi-proxy.ts status          # one profile: --profile not needed
node bin/nhi-proxy.ts ca --spki

node bin/nhi-proxy.ts setup --site https://staging.koumoul.com < /dev/null
node bin/nhi-proxy.ts status          # two profiles: must refuse and list them
node bin/nhi-proxy.ts status --profile koumoul.com
```
...
Write the README covering, in this order: what nhi-proxy is and the problem it solves; the **quickstart** (`npx @data-fair/nhi-proxy` and nothing else — it sets up on first run and serves thereafter); the enrollment walkthrough (`setup` → admin pastes → `enroll`); **profiles** (named after the site host, implicit when there is only one, `--profile` required past that, `nhi-proxy profiles` to list); the wiring recipes; the two silent-failure gotchas; the three key-protection tiers with the caveat; and troubleshooting keyed to the diagnostic messages.
...
- **curl:** `curl --proxy http://127.0.0.1:7331 --cacert ~/.config/nhi-proxy/koumoul.com/ca.crt https://koumoul.com/data-fair/api/v1/datasets`
- **Node tools:** `HTTPS_PROXY=http://127.0.0.1:7331` and `NODE_EXTRA_CA_CERTS=~/.config/nhi-proxy/koumoul.com/ca.crt`
- **Playwright MCP**, preferred form (pins one key rather than disabling validation):
  ...
  ```
  with `--ignore-https-errors` documented as the fallback **and its cost stated**: it stops the browser validating certificates for every site in that session, including the ones nhi-proxy tunnels and never touches.
...
node bin/nhi-proxy.ts setup --site https://koumoul.com < /dev/null
node bin/nhi-proxy.ts profiles
node bin/nhi-proxy.ts ca
node bin/nhi-proxy.ts --help
```
...
export XDG_CONFIG_HOME=$(mktemp -d)
node bin/nhi-proxy.ts setup --site https://koumoul.com < /dev/null
PIN=$(node bin/nhi-proxy.ts ca --spki)
...
to `nhi-proxy ca --spki` and document that value instead. Record the outcome in
...
- `npx @data-fair/nhi-proxy` on a machine with no profile walks a new user from the first prompt to a running proxy without them reading the README.
